...
```
It should have 4237 rows and 124 columns:
```{r eval = TRUE}
dim(newphos)
```
The first two columns are `gene_symbol` and `variable_sites_names` that we will
use to create row names for the PTM table, and the remaining columns are samples
with phosphoproteomic data. Here are the first few rows and columns of the data frame:
```{r eval = TRUE}
head(newphos[, 1:5])
```
...
<!-- it is confusing how this works, since name.peptide is in RawDatProcessing.R, but we don't export that function -->
...
Create `ptmtable` with PTMs as rows and samples as columns for use in the next steps, and remove the columns we used to create the row names.
```{r eval = TRUE}
...
Load in pre-computed results:
```{r eval = TRUE}
clusterlist.data <- brca_clusterlist_data
CCCN.data <- brca_CCCN_data
```

Whether computed or loaded, the results are lists that contain the following elements:
```{r eval = TRUE}
...
<!-- Lucy note: I don't know where brca_clusters, brca_tbl, came from -->
<!-- Lucy note: for the vignette, can we just print these to the screen instead of saving? -->
