...
Per their description, the `HDBSCAN;min_cluster_size-4` column assigns a module number to each phosphosite.
...
head(PD_module.df)
```
Note differences from the PTM table imported above. To compare, we need to make them match.
```{r eval = TRUE}
...
```
Let's get the unique genes in each module to compare to the unique genes in our clusters.
```{r eval = TRUE}
...
Let's see which of our clusters have intersections with module 63. We will use this as an example to show how to
explore the networks around a particular module of interest.
...
P2P provides functions to prepare visualizations of these PTMs in Cytoscape. This code graphs the CFN/CCCN from all these PTMs:
```{r eval = TRUE}
