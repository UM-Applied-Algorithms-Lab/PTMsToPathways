...
- cccn_matrix_genes
  - dataframe of dataframes that represent the common clusters from the three distance calculations' clusters (has been converted to genes that encode the proteins for analysis)
...
```{r echo = FALSE}
#PLEASE *****NOTE***** everything has to be converted into genes in order to do analysis but will have to be converted back into proteins to return important results.
#consider: dictionary linking proteins to genes and vice versa; pray for no overlap
```
...
- cccn_matrix_genes
  - dataframe returned by the step 3: Find PPI Edges; dataframe of dataframes that represent the common clusters from the three distance calculations' clusters (has been converted to genes that encode the proteins for analysis)  
