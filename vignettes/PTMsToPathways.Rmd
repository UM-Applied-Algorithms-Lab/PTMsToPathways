...
example used in the [Raw Data Processing vignette](vignette:RawDataProcessing)) and a larger dataset
...
the [Raw Data Processing vignette](vignette:RawDataProcessing)for a tutorial showing all steps needed
...
Next, we look at a piece of the `adj.consensus` table. Ones represent a pair
...
Instructions on saving the correct GeneMANIA file are shown below. Click on the three lines in the upper right corner. This should be under the GeneMANIA side window beside the species. Click "Export Results". The path to this file is the gm.results.path:
...
kinasesubstrate.filename <- system.file("extdata", "Kinase_Substrate_Dataset.txt", package = "PTMsToPathways")
```
