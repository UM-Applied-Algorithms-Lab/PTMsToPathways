...
```{r eval = FALSE}
ProcessGMEdgefile(gm.edgefile.path, gm.nodetable.path, db_nodes.path, genemania.edges.name = "genemania.edges")
```
...
```{r eval = FALSE}
network.list <- BuildClusterFilteredNetwork(stringdb.edges, genemania.edges, gene.cccn, db.filepaths = c(), ppi.network.name = "combined.PPIs", cfn.name = "cfn")
...
```{r eval = FALSE}
PathwayCrosstalkNetwork(common.clusters, bioplanet.file = "bioplanet.csv", edgelist.name = "PCNedgelist", jaccard.edgelist.name = "jaccard.net", CPE.edgelist.name = "CPE.net", PCN.edgelist.name = "pathway crosstalk network", createfile = getwd())
```
