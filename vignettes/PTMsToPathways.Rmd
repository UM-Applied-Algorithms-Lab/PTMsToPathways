...
dataset consisting of 933 PTMs and 18 experimental conditions (the example used in the RawDataProcessing vignette) and a
...
can be downloaded from our GitHub page ------- to the user's working directory
and imported.
...
\|Spearman Correlation\|), and the average of both Spearman Dissimilarity (1-
Spearman Correlation) and Euclidean Distance (SED). Combining the
...
calculated distance and provides you with a cluster list, *common.clusters*. The returned *adj.consuensus* and *ptm.correlation.matrix* are also used in the next step to create co-cluster correlation networks (CCCNs).
...
step requires input of an external database, bioplanet (https://tripod.nih.gov/bioplanet/download/pathway.csv), that contains
