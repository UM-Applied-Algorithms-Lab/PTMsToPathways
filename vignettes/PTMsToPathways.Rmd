...
R supports many file types and can automatically convert them into a data frame. For example, read.csv() will take a csv file and convert it into a data frame. It is recommended to work .csv specifically as that is what is what we mainly used when designing this package.
...
#### Turning PTMs from columns into rownames
If you have one (or multiple) columns that contain parts of PTM names. This is a command for concatenating those columns and turning them into data frame. The vector "name.columns" is the indices of the columns which contain names. For the example below, the strings in columns 1, 2, 3, for each row, will be concatenated and then turned into a rowname. It is also possible to have name.columns equal a vector like c(2, 4, 6), which takes columns 2, 4, and 6. 
...
#### Replacing Patterns
If your list of PTMs possesses symbols that are unnecessary like "AARS ~K ubi K747", this command will remove all strings included in the "patterns" vector from your rownames. You can edit it as you like, just make sure that any special character such as $ or @ have a "\\\\" in front of them. 
...
Make Correlation Network filters a correlation matrix of PTMs by specific PTMs. It groups the PTM correlation matrix based on the Genes of PTMs. By summing these submatrices, it produces a gene by gene cocluster correlation network shows strength of relationships between proteins using the common clusters between the three distance metrics.  
...
- ptm.cccn (or otherwise named by ptm.cccn.name)  
  - The PTM correlation matrix filtered by specific PTMs from input.
- gene.cccn (or otherwise named by gene.cccn.name)  
  - A matrix showing strength of relationships between proteins using the ptm.cccn.
