...
"Transmembrane transport of small molecules" and those pathways. The utility
functions used below are in CytoscapeGraphing.R
...
We adopt the following standards for visualizing nodes and edges in Cytoscape. The border and shape of the node represent the type of protein this gene is, based on a function key that maps gene names to a table of information regarding the gene. This may be provided by the user or PTMsToPathways provides an example dataset as [function_key](reference/function_key.html).

```{r visible-table eval = FALSE, echo = TRUE}
head(function_key)
```

```{r table-run eval = TRUE, echo = FALSE}
