...
numeric data in all the columns, which can be used as input to the PTMsToPathways
functions.
...
R converts spaces in column names to periods, so the relevant columns after
import are:
...
For input to PTMsToPathways, it important that the row names are PTM identifiers and that the columns
represent experimental conditions. The numeric values are the mass
spectrometer output, and NAs represent missing data rather than zeroes.
Ambiguous PTMs, where a PTM could match several proteins, are separated by
...
reverse that with the helper function [`fix.excel()`](reference/fix.excel.html).
...
averaging values detected in both, using the PTMsToPathways function [merge2cols()](reference/merge2cols.html).
...
For subsequent steps, we make a dataframe withjust the data with individual PTMs as rownames:
```{r}  
