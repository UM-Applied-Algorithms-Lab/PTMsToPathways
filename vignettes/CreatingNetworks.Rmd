...
vignette](BCRANetworks.html) demonstrates how to get the STRING-db and
GeneMANIA edges from the static downloaded networks.
...
The final step is the creation of the Pathway Crosstalk Network (PCN), which
creates a set of pathway-pathway edges that have two weights: a Jaccard similarity
and a Cluster-Pathway Evidence score.
...
processes known as pathways. P2P provides a function [`ReadBioplanetFile`](references/ReadBioplanetFile.html)
that reads in the BioPlanet file and converts it into a list of pathways. The first pathways is displayed below:
...
sum(erbb_clusts)
```
And overall, here are the number of clusters that contain PTMs on at least one protein from both the `Axon guidance` pathway and the `ERBB signaling pathway`:
```{r eval = TRUE, echo = TRUE}
