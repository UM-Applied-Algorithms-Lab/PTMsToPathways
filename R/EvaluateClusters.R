...
EvaluateClusters <- function(
    ...
    .gene.names <- function(cl.el) {
        if (is.data.frame(cl.el)) {
            if ("PTMnames" %in% names(cl.el)) {
                return(cl.el[["PTMnames"]])
            }
            if ("Gene.Name" %in% names(cl.el)) {
                return(cl.el[["Gene.Name"]])
            }
            stop("Cluster data frame must contain a 'PTMnames' or 'Gene.Name' column.")
        }
        if (is.character(cl.el)) {
            return(cl.el)
        }
    ...
    for (i in seq_len(n.clusters)) {
        ...
        if (length(acol) <= 1L || length(gene.names) == 1L) {
            ...
            if (use.slope) {
                ...
                # assessment.
                col.order <- order(-vapply(
                    at, function(x) sum(x, na.rm = TRUE),
                    numeric(1L)
                ))
                row.order <- order(-vapply(seq_len(nrow(at)), function(r) {
                    sum(as.numeric(at[r, ]), na.rm = TRUE)
                }, numeric(1L)))
    ...
    output.cols <- c(
        "Group", "no.genes", "culled.by.slope",
