...
#'
#' Converts Bioplanet pathways from (<https://tripod.nih.gov/bioplanet/>) into a list of pathways whose elements are the genes in each pathway. Edge weights are either the PTM Cluster Weight or the Jaccard Similarity.
#'
...
#' @param bioplanet.file Either the name of the bioplanet pathway .csv file OR the name of a dataframe. Lines of bioplanet should possess 4 values in the order "PATHWAY_ID","PATHWAY_NAME","GENE_ID","GENE_SYMBOL". Users not well versed in R should only pass in "yourfilename.csv"
...
#' @return An edgelist file that is created in the working directory and saved to the global environment. Contains pathway source-target columns, with edge weights of their jaccard similarity and their Pathway-Pathway Evidence score
#' The CPE.matrix which represents how likely it is that a pathway interacts with a given cluster. The Jaccard.Full matrix that measures the Jaccard similarity between pairs of pathways is also returned.
#' @export
#'
#' @examples
