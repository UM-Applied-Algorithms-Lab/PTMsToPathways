...
PathwayCrosstalkNetwork <- function(file = "bioplanet.csv", clusterlist, edgelist.name = "PTPedgelist"){
  ...
  CPE <- apply(PTPedgelist, 1, function(x) sum(CPE.matrix[,x[1]] + CPE.matrix[,x[2]], na.rm=TRUE)) #Get a vector of all the CPE weights for every pair of pathways. For a CPE weight to be non-NA, they BOTH must have a nonzero CPE for the at least 1 cluster.
