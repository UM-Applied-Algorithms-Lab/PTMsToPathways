...
#
setEdgeWidths.new <- function (ffactor=10, log=FALSE)	{
...
make.cytoscape.node.file <- function(edge.file, funckey, ptmtable, include.ptm.data = FALSE) {
  ...
  # Step 3: Optionally merge PTM data
...
GraphCfn <- function(cfn.edges, cfn.nodes, plotcol,  Network.title = "CFN", Network.collection = "PTMsToPathways", visual.style.name = "PTMsToPathways.style",
                        background.color = "#949494", edge.label.color = '#17202a', node.label.color = '#000000',
                        default.font = "Arial", node.font.size = 20, edge.font.size = 8,
                        edge.line.style = 'SOLID',
                        edge.opacity = 175, edge.label.opacity = 255, border.opacity = 255, node.label.opacity = 255, node.fill.opacity = 255
