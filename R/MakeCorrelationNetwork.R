      ...
      for(c in 1:length(list3.ptms)){
        ...
        if(length(temp) > klength) {
          ...
          if(length(ambiguous.PTMs != 0)){ #If an ambiguous PTM was found
            ...
            ambiguous[[length(ambiguous)+1]] <- a.temp
          }
...
#' @param common.clusters.name #The desired name for the output of the list containing common and ambigious clusters
...
MakeCorrelationNetwork <- function(tsne.matrices, ptm.correlation.matrix, keeplength = 2, common.clusters.name = "common.clusters", cccn.name = "cccn_matrix"){
  ...
  common.clusters <- FindCommonClusters(tsne.matrices[[1]], tsne.matrices[[2]], tsne.matrices[[3]], keeplength)
  list.common <- common.clusters[[1]]

  # Generate the combined adjacency matrix
  ulist <- unique(unlist(list.common)) # Use this for rownames and colnames

  cccn_matrix <- matrix(NA, nrow=length(ulist), ncol=length(ulist), dimnames=list(ulist, ulist)) # Initialize empty matrix
  ...
  cccn_matrix[is.na(cccn_matrix)] <- 0                              # make NAs zero again
  assign(common.clusters.name, common.clusters, envir = .GlobalEnv) # Return list of common clusters
  assign(cccn.name, cccn_matrix, envir = .GlobalEnv)                # Return cccn_matrix (matrix containing Euclidean t-SNE coords)
