fix.excel <- function(vector) {  
  ...
  indeces <- vector %in% names(correct.to) #indecies that need to be fixed
  fixme <- vector[indeces] #create a temp vec
  
  if(length(fixme) != 0) for(i in 1:length(fixme)) fixme[[i]] <- correct.to[[fixme[[i]]]] #Correct them
  
  vector[indeces] <- fixme #Replace everything with temp vec
